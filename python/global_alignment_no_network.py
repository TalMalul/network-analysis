...
from tutils.sequence_alignment import get_global_score
...
def _main():
	...
	for idx_source in range(options.start, stoping_index, 1):
		...
		for idx_target in range(idx_source + 1, len(database.dataframe.index)):
			...
			alignment = get_global_score(source_record, target_record)
			alignment_identity = alignment['%global_id']
			normalized_alignment_score = alignment['normalized_global_score']
			alignment_score = alignment['global_score']
